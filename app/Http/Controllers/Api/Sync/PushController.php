...
{
    return $this->handlePushGlobal($request, 'customers', function ($records) {
        return $this->pushService->processCustomers($records);
    });
}

    private function handlePushGlobal(Request $request, string $entity, callable $processor)
    {
        $startTime = microtime(true);
        $records   = $request->input('records', []);
        ...
        if (count($records) > SyncPushService::MAX_BATCH_SIZE) {
            return $this->error(
                'Máximo ' . SyncPushService::MAX_BATCH_SIZE . ' registros por batch.',
                422
            );
        }
        ...
        try {
            ...
            SyncLog::logPush(
                0,  // sin branch_id para entidades globales
                $entity,
                count($records),
                count($accepted),
                count($rejected),
            ...
            $durationMs = (int) ((microtime(true) - $startTime) * 1000);
            SyncLog::logPush(0, $entity, count($records), 0, count($records), 'error', $e->getMessage(), $durationMs);
            return $this->error('Error procesando batch: ' . $e->getMessage(), 500);
        }
    }
}